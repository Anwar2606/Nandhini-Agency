...
const downloadAllCopies = (bill) => {
    ...
    const tamilFontBase64 = "4KqN4K6J4KqH4K6A4KqH4K6y4Kq54Kqg4K6q4KqV";
    jsPDF.API.events.push(['addFonts', function() {
        this.addFileToVFS("Latha.ttf", tamilFontBase64);
        this.addFont("Latha.ttf", "Latha", "normal");
    }]);
    // Loop through each copy type and generate the PDF pages
    copies.forEach((copyType, index) => {
      // Start a new page for each copy after the first one
      if (index > 0) {
        doc.addPage();
      }
...
//   const formattedDate = formatDate(bill.createdAt);
  ...
  doc.rect(borderMargin, borderMargin, pageWidth - 2 * borderMargin, pageHeight - 2 * borderMargin);
      // Add company header and invoice details
      ...
      doc.setFontSize(8);
      doc.text("Sri Ponerulappa Swami Thunai", 90, 8.5);
      doc.setFontSize(10);
      doc.setTextColor(255, 0, 0);
      doc.setFont('helvetica', 'bold');
      doc.text('NANDHINI AGENCY', 44, 21);
      doc.setTextColor(0, 0, 0);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8.5);
      doc.text('2/156, Vanamoorthilingapuram, Sivakasi-626 123', 44, 28);
      doc.setFontSize(9);
      doc.text('Phone number:', 44, 35);
      doc.setFont('helvetica', 'normal');
      doc.text('+91 97867 69539', 68, 35);
      doc.setFont('helvetica', 'bold');
      ...
      doc.text(copyType.toUpperCase() , 138, 29);
      doc.setTextColor(0, 0, 0);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.text(`Invoice Number: NA-${bill.invoiceNumber || 'N/A'}-25`, 138, 43);
      doc.text(`Date: ${formattedDate}`, 138, 36);
      doc.setFont('helvetica', 'bold');
      doc.text('GSTIN: 33AAMFN7599K1ZF', 138, 49);
  
      // Rectangle for customer and shipping details
    ...
    //     doc.text(':', startX + 20, startY);
    //     doc.text(value, startX + 24, startY);
    //     startY += lineHeight;
    //   });
    const customerDetails = {
        Name: bill.customerName || 'N/A',
      ...
      // Repeat for SHIPPED TO section
    const shippedDetails = {
  AccountHolderName: "NANDHINI AGENCY",
  AccountNumber: "008502000011486",
  BankName: "INDIAN OVERSEAS BANK",
  Branch: "SIVAKASI",
  IFSCCode: "IOBA0000085"
};
      ...
      // Loop through shipped details
      Object.entries(shippedDetails).forEach(([label, value]) => {
        doc.text(label, initialX, initialY);
        doc.text(':', initialX + 30, initialY);
        doc.text(value, initialX + 34, initialY);
        initialY += lineHeight;
      });
      
      // Draw rectangle around the sections
      const rectX = 14;
      const rectY = 58;
      const rectWidth = 182;
      const rectHeight = 75;
      ...
      // Draw a vertical line to separate BILLED TO and SHIPPED TO
    ...
    //   ]);
const tableBody = bill.productsDetails.map(item => {
  const saleprice = typeof item.saleprice === 'string' || typeof item.saleprice === 'number' ? item.saleprice : 0;
  const quantity = typeof item.quantity === 'string' || typeof item.quantity === 'number' ? item.quantity : 0;
  ...
  return [
    ...
    quantity.toString(),
    `Rs.${saleprice}`,
    `Rs.${(quantity * saleprice)}`
  ];
});

// Add an empty row as a separator before totals
tableBody.push(['', '', '', '', '']);

// ✅ Summary values
const totalAmount = bill.totalAmount ? `Rs.${bill.totalAmount}` : 'N/A';
const discountedAmount = bill.discountedTotal ? `Rs.${bill.discountedTotal}` : 'N/A';
const cgstAmount = bill.cgstAmount ? `Rs.${bill.cgstAmount}` : 'N/A';
const sgstAmount = bill.sgstAmount ? `Rs.${bill.sgstAmount}` : 'N/A';
const grandTotal = bill.grandTotal ? `Rs.${bill.grandTotal}` : 'N/A';

// ✅ Add summary rows inside the same table
tableBody.push(
  ['Total Amount', '', '', '', totalAmount],
  // ['Discounted Amount', '', '', '', discountedAmount], // uncomment if needed
  ['CGST (9%)', '', '', '', cgstAmount],
  ['SGST (9%)', '', '', '', sgstAmount],
  ['Grand Total', '', '', '', grandTotal]
);

// ✅ Single combined table with black borders
doc.autoTable({
  ...
  didParseCell: function (data) {
    ...
    if (
      ...
      const grandTotalInWords = `Rupees: ${convertNumberToWords(bill.grandTotal || 0)}`;
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(10);
      doc.text(grandTotalInWords, 15, pageHeight - 60);
  
      // Terms and Conditions (You can add more as needed)
      const termsAndConditions = [
        'Terms & Conditions',
        '1. Goods once sold will not be taken back.',
        '2. Subject to jurisdiction of Sivakasi.'
      ];
      doc.setFontSize(8);
      termsAndConditions.forEach((line, i) => {
        doc.text(line, 15, pageHeight - 40 + i * 5);
      });
    });
